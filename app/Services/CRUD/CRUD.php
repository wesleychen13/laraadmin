...
class CRUD extends BaseProcess
{
    ...
    public function _dealData($data)
    {
        ...
        $data['CriteriaMultiWherePath'] = app_path() . "/Repositories/" . substr($data['repositories'], 0, strrpos($data['repositories'], "Repository")) . "/Criteria/MultiWhere" . ".php";;
        $data['CriteriaMultiWhereUse'] =  'App\Repositories\\' .str_replace('/', '\\', substr($data['repositories'], 0, strrpos($data['repositories'], "Repository"))) . "\\Criteria\\MultiWhere";
    ...
    public function _addCriteriaMultiWhere()
    {
        ...
        $queryKeyord = "";
        foreach ($this->_fieldData as $k =>$data) {
            if($data->DATA_TYPE != 'timestamp'){
                if($k == 0){
                    $queryKeyord .= " if(isset(\$this->search['keyword']) && \$this->search['keyword']) {
                                    \$model = \$model->where('{$data->COLUMN_NAME}','like','%'.\$this->search['keyword'].'%')\r\n";

                }else{
                    $queryKeyord .=   " ->orWhere('{$data->COLUMN_NAME}','like','%'.\$this->search['keyword'].'%')\r\n";
                }
            }
        ...
        $str = str_replace("{{sortPath}}", str_replace('/', '\\', substr($this->_data['repositories'], 0, strrpos($this->_data['repositories'], "Repository"))), $str);
    ...
    public function _addView()
    {
            ...
            if (!in_array($data->COLUMN_NAME, ["deleted_at"]) && $data->DATA_TYPE != 'text') {
                ...
                switch ($data->DATA_TYPE) {
                    case "tinyint":
                        ...
                        break;
                    default:
                        if ($data->COLUMN_NAME == 'image' || $data->COLUMN_NAME == 'pic' || $data->COLUMN_NAME == 'picture' || $data->COLUMN_NAME == 'photo' || $data->COLUMN_NAME == 'avatar' || $data->COLUMN_NAME == 'img'){
        ...
        foreach ($this->_fieldData as $key => $data) {
            ...
            switch ($data->DATA_TYPE) {
                case "tinyint":
                    if (dict()->get($this->_data['table'], $data->COLUMN_NAME)) {
                        $viewTdStr .= "<span class='label label-primary'>{{ dict()->get('{$this->_data['table']}','{$data->COLUMN_NAME}',\$data['{$data->COLUMN_NAME}']) }}</span>";

                    } elseif (dict()->get('global', $data->COLUMN_NAME)) {
                        $viewTdStr .= "<span class='label label-primary'>{{ dict()->get('global','{$data->COLUMN_NAME}',\$data['{$data->COLUMN_NAME}']) }}</span>";
                ...
                default:
                    if ($data->COLUMN_NAME == 'image' || $data->COLUMN_NAME == 'pic' || $data->COLUMN_NAME == 'picture' || $data->COLUMN_NAME == 'photo' || $data->COLUMN_NAME == 'avatar' || $data->COLUMN_NAME == 'img'){
                        $viewTdStr .= "<img src=\"{{ \$data['{$data->COLUMN_NAME}'] }}\"  style='height: 80px'>";
                    }else{
                        $viewTdStr .= " {{ \$data['{$data->COLUMN_NAME}'] or ''}}";
                    }
